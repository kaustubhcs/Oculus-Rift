...
// Import marker detection functions
    ...
    void Update () 
	{
        if (m_WebcamTexture && m_WebcamTexture.didUpdateThisFrame) 
		{
	        ...
	        // Reset rendering
	        if (m_3dGameObjectCG1)
	        {
	            Renderer[] renderers = m_3dGameObjectCG1.GetComponentsInChildren<Renderer>();
	        ...
	        if (iNbrOfDetection > 0)
	        {
	            for (int i=0; i<iNbrOfDetection; i++)
	            {
	                ...
	                if (m_3dGameObjectCG1)
	                {
	                    ...
	                    m_3dGameObjectPivot1.transform.position = position;
						m_3dGameObjectPivot1.transform.rotation = quat;
	                }
